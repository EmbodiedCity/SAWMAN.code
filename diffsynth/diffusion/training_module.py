...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def _named_state_members(self):
        state_members = {}
        state_members.update(dict(self.named_parameters()))
        state_members.update(dict(self.named_buffers()))
        return state_members

    def _load_state_dict_zero3(self, state_dict):
        import deepspeed

        state_members = self._named_state_members()
        missing_keys = []
        unexpected_keys = [key for key in state_dict if key not in state_members]
        mismatched_keys = []
        is_distributed = torch.distributed.is_available() and torch.distributed.is_initialized()
        rank = torch.distributed.get_rank() if is_distributed else 0

        for name, target in state_members.items():
            ...
            if isinstance(target, torch.nn.Parameter) and hasattr(target, "ds_id"):
                gather_context = deepspeed.zero.GatheredParameters([target], modifier_rank=0)

            with gather_context:
                target_tensor = target.data if isinstance(target, torch.nn.Parameter) else target
                if tuple(target_tensor.shape) != tuple(source.shape):
                    mismatched_keys.append((name, tuple(source.shape), tuple(target_tensor.shape)))
                    continue

                if rank == 0:
                    source_tensor = source.to(device=target_tensor.device, dtype=target_tensor.dtype)
                    target_tensor.copy_(source_tensor)

        if mismatched_keys:
            mismatch_lines = [
                f"\tsize mismatch for {name}: copying a param with shape {source_shape} from checkpoint, the shape in current model is {target_shape}."
                for name, source_shape, target_shape in mismatched_keys
            ]
            raise RuntimeError(
                "Error(s) in loading state_dict for sharded DiffusionTrainingModule:\n" + "\n".join(mismatch_lines)
            )
    ...
    def resume_from_checkpoint(self, path, remove_prefix_in_ckpt):
        ...
        if is_deepspeed_zero3_enabled():
            missing_keys, unexpected_keys = self._load_state_dict_zero3(state_dict)
        else:
