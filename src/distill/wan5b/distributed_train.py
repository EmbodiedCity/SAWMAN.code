...
All networks retain the SFT's bidirectional attention and native TI2V first-frame
fusion. Rank-local FP32 shards are optimized directly; BF16 is only compute.
"""
import argparse
import copy
...
class NativeWan(torch.nn.Module):
    ...
    def forward(self, x, timestep, context, checkpointing=False):
        return model_fn_wan_video(
            dit=self.dit, latents=x.to(torch.bfloat16), timestep=timestep.to(torch.bfloat16),
            context=context.to(torch.bfloat16), fuse_vae_embedding_in_latents=True,
            use_gradient_checkpointing=checkpointing,
        ).float()


def velocity(model, x, sigma, case, checkpointing=False):
    x = common.clamp_first(x, case["first"])
    t = torch.tensor([float(sigma) * 1000], device=x.device, dtype=torch.bfloat16)
    return model(x, t, case["context"], checkpointing)


def x0(model, x, sigma, case, checkpointing=False):
    x = common.clamp_first(x.float(), case["first"])
    if float(sigma) == 0:
        return x
    return common.clamp_first(x - float(sigma) * velocity(model, x, sigma, case, checkpointing), case["first"])
...
def dmd_gradient(generated, real_x0, fake_x0):
    ...
    scale = (generated[:, :, 1:] - real_x0[:, :, 1:]).abs().mean(dim=(1, 2, 3, 4), keepdim=True).clamp_min(1e-3)
    gradient = (fake_x0[:, :, 1:] - real_x0[:, :, 1:]) / scale
...
def rollout(student, case, sigmas, exit_index, with_grad):
    x = common.clamp_first(torch.randn_like(case["clean"], dtype=torch.float32), case["first"])
    for i in range(exit_index + 1):
        ...
        x = common.clamp_first((1 - following) * clean.detach() + following * torch.randn_like(x), case["first"])
...
def stage2_step(student, teacher, ema, optimizer, case, cfg, step):
    ...
    x = common.clamp_first((1 - sigma) * clean + sigma * torch.randn_like(clean), case["first"])
    with torch.no_grad():
        endpoint = common.clamp_first(x - (sigma - following) * velocity(teacher, x, sigma, case), case["first"])
        target = x0(ema, endpoint, following, case)
    predicted = x0(student, x, sigma, case, checkpointing=True)
    loss = (predicted[:, :, 1:] - target[:, :, 1:]).square().mean()
...
def stage3_step(student, teacher, ema, fake, optimizer, fake_optimizer, case, cfg, step):
    ...
    noisy = common.clamp_first((1 - sigma) * generated.detach() + sigma * noise, case["first"])
    generator_loss = 0.0; generator_norm = 0.0
    if update_generator:
        with torch.no_grad():
            real_x0 = x0(teacher, noisy, sigma, case)
            fake_x0 = x0(fake, noisy, sigma, case)
            grad = dmd_gradient(generated.detach(), real_x0, fake_x0)
            target = generated[:, :, 1:].detach() - grad
    ...
    fake_loss = (fake_velocity[:, :, 1:] - flow_target[:, :, 1:]).square().mean()
    fake_loss.backward()
...
def evaluate_saved(destination, cfg, ema, teacher, pipe, cases, rows, device):
    ...
    for kind, model, steps in (("teacher", teacher, 50), ("student", ema, cfg["inference_steps"])):
        ...
        values = common.clamp_first(values, case["first"])
        sigmas = common.grid(steps, cfg["sigma_shift"])
        for i in range(steps):
            sigma, following = float(sigmas[i]), float(sigmas[i + 1])
            if kind == "teacher":
                values = common.clamp_first(values - (sigma - following) * velocity(model, values, sigma, case), case["first"])
            else:
                clean = x0(model, values, sigma, case)
                if following:
                    noise = torch.randn(values.shape, dtype=torch.bfloat16, generator=generator).to(device).float()
                    values = common.clamp_first((1 - following) * clean + following * noise, case["first"])
                else:
                    values = clean
        if not torch.equal(values[:, :, :1], case["first"].float()):
            raise AssertionError("Evaluation changed the observed latent")
...
def verify_generator(cfg, allow_pending=False):
    ...
    for key in ("teacher_checkpoint", "metadata", "height", "width", "num_frames", "sigma_shift", "inference_steps", "teacher_guidance"):
    ...
    return "matched completed navigation Stage2"
...
def main():
    ...
    if args.stage == "stage3":
        if cfg["critic_updates_per_generator"] < 1 or cfg["critic_learning_rate"] <= 0:
            raise ValueError("Invalid DMD critic configuration")
        status = verify_generator(cfg, args.preflight and args.allow_pending_stage2)
    else:
        status = "teacher/student/EMA initialize directly from navigation SFT"
    if args.preflight:
    ...
    base = pipe.dit
    pipe.dit = None
    initialization = cfg.get("generator_checkpoint") if args.stage == "stage3" else None
    ...
    del base; gc.collect()
    pipe.device = device
    pipe.vae.to(device); pipe.text_encoder.to(device)
